...
function DoPlayerEntitySpawn( player, entity_name, model, iSkin, strBody )
	...
	ent:SetNWEntity("PlayerOwnerEnt", player) 			-- THIS IS ADDED FOR M.B.D.; VERY IMPORTANT to track which user that bought a prop strictSetting ( on client side )
	ent:SetNWString("PlayerOwner", player:UniqueID()) 	-- THIS IS ADDED FOR M.B.D.; VERY IMPORTANT to track which user that bought a prop strictSetting
	setEntColorTransparent(ent, "fromSpawnMenu")		-- THIS IS ADDED FOR M.B.D.; VERY IMPORTANT to track which user that bought a prop strictSetting

	ent:Spawn()
	ent:Activate()

	registerAnEntityForUndoLater("M.B.D. Player Prop ("..string.lower(ent:GetModel())..")", ent, player)

	-- - -
	-- Remove the entity, if it is strict mode and it is not on the list >> >
	local ID = "mbd:CheckIfAllowed001"..ent:EntIndex()
	timer.Create(ID, 0.5, (0.5 * 10), function()
		if (
			GetConVar("mbd_enableStrictMode"):GetInt() == 1 and
			ent and
			ent:IsValid() and
			string.match(ent:GetName(), "mbd_")
		) then
			timer.Remove(ID)
			-- Only check prop_physics !!
			if ent:GetClass() == "prop_physics" then
				...
				if !Model then
					if (
						ent and
						ent:IsValid()
					) then undoEntityWithOwner(player, ent, "Returned a Prop (invalid model)") return false end
				end
...
function CCGiveSWEP( player, command, arguments )
	...
	if ( !player:HasWeapon( swep.ClassName ) ) then
...
function Spawn_Vehicle( Player, vname, tr )
	...
	MBDAddSomeMBDStuffAVehicle(Ent, vehicle.Class)
...
-- MODDDE MBD
concommand.Add( "mbd_gm_spawnvehicle", function( ply, cmd, args ) SendLocalSoundToAPlayer("prop_spawn", ply) Spawn_Vehicle( ply, args[1] ) end )
