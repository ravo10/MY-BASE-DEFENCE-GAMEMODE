...
local function CreateSpawnMenu()
	...
	-- first.
	if (strictSetting == 0) then hook.Run( "AddToolMenuTabs" ) end

	-- Use this hook to add your custom tools
	-- We add the gamemode tool menu categories first
	-- to ensure they're always at the top.
	-- MOD THIS ONE FOR TABS-CONTROL
	hook.Run( "AddGamemodeToolMenuCategories" )
	if (strictSetting == 0) then hook.Run( "AddToolMenuCategories" ) end
...
function GM:OnSpawnMenuOpen()
	...
	timer.Remove( "mbd_ResetClSpawnmenuIconSize_timer" )
	...
	if ( IsValid( g_SpawnMenu ) ) then
		...
		timer.Create( "mbd_ResetClSpawnmenuIconSize_timer", 0.06, 1, function()
		
			MBDResetClSpawnmenuIconSize = false
...
function GM:OnSpawnMenuClose()
	...
	if ( IsValid( g_SpawnMenu ) ) then MBDResetClSpawnmenuIconSize = true g_SpawnMenu:Close() end
