...
if strictSetting == 0 then
	hook.Add( "PopulateWeapons", "AddWeaponContent", function( pnlContent, tree, node )
		...
		for k, weapon in pairs( Weapons ) do
			...
			if ( !weapon.Spawnable ) then CONTINUE = true end
	
			
			if !CONTINUE then Categorised[ weapon.Category ] = Categorised[ weapon.Category ] or {}
				table.insert( Categorised[ weapon.Category ], weapon ) end
		end
