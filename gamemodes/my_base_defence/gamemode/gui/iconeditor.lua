...
function PANEL:FillAnimations( ent )
	...
	for k = 0, ent:GetNumBodyGroups() - 1 do
		...
		if ( ent:GetBodygroupCount( k ) <= 1 ) then CONTINUE = true end

		if !CONTINUE then local combo = self.BodyList:Add( "DComboBox" )
			...
			combo.OnSelect = function( pnl, index, value, data ) data() end end
	end
