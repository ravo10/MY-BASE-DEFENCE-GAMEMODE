	...
	init		= function( icon, window )
		...
		local function MakeNiceName( str )
			...
			for _, s in pairs( string.Explode( "_", str ) ) do
				...
				if ( string.len( s ) == 1 ) then table.insert( newname, string.upper( s ) ) CONTINUE = true end
				
				if !CONTINUE then table.insert( newname, string.upper( string.Left( s, 1 ) ) .. string.Right( s, string.len( s ) - 1 ) ) --[[ Ugly way to capitalize first letters. ]] end
			end
		...
		local function RebuildBodygroupTab()
			...
			for k = 0, mdl.Entity:GetNumBodyGroups() - 1 do
				...
				if ( mdl.Entity:GetBodygroupCount( k ) <= 1 ) then CONTINUE = true end

				if !CONTINUE then local bgroup = vgui.Create( "DNumSlider" )
					bgroup:Dock( TOP )
