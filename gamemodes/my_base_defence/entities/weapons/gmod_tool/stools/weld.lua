...
function TOOL:LeftClick( trace )
	...
	if ( iNum == 1 ) then
		...
		--- MBD Specs.. >>
		if (
			Ent1:GetCollisionGroup() == COLLISION_GROUP_VEHICLE or
			Ent2:GetCollisionGroup() == COLLISION_GROUP_VEHICLE
		) then
			-- Do itt
			Ent1:SetNWBool("isWeldedToVehicle", true)
			Ent2:SetNWBool("isWeldedToVehicle", true)
		end
...
function TOOL:Reload( trace )
	...
	for _, v in pairs(constraint.GetAllConstrainedEntities(trace.Entity)) do
		...
		if (
			...
			-- Do itt
			v:SetNWBool("isWeldedToVehicle", false)
		end
	end
