...
function SWEP:DrawHUD()
	...
	for k, v in pairs( self:GetToolObject().Information ) do
		...
		if ( !v.name ) then CONTINUE = true end
		if ( v.stage && v.stage != self:GetStage() ) then CONTINUE = true end
		if ( v.op && v.op != self:GetToolObject():GetOperation() ) then CONTINUE = true end

		if !CONTINUE then local txt = "#tool." .. GetConVarString( "gmod_toolmode" ) .. "." .. v.name
			if ( v.name == "info" ) then
