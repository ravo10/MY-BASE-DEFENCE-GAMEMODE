...
SWEP.PrintName      = "MECHANICAL"
SWEP.Author			= "ravo (Norway)"
SWEP.Contact		= ""
SWEP.Purpose		= "To repair a Vehicle (M.B.D. Gamemode)"
SWEP.Instructions   = [[
    Left-Click: Repair traced Vehicle (aim for middle)

    Hold "Speed Bound" Key (def. Shift) while letting go of Left-Click to Point (hold "USE KEY" together, to show location).
    ]]
if CLIENT then
    SWEP.WepSelectIcon = surface.GetTextureID("vgui/mbd/swep_vehicle_repair/swep_vehicle_repair")
end
...
local function swepCleanUp(self)
    -- Clean up
    timer.Remove("mbd:SwepVehicleRepairAnimation001")
    timer.Remove("mbd:SwepVehicleRepair001")
...
if CLIENT then
    ...
    hook.Add("HUDPaint", "mbd:SWEPSight:VehicleRepair001", function()
        ...
        if (
            ...
            SWEPOwner:GetActiveWeapon():GetClass() == "swep_vehicle_repair" and (
                string.match(SWEPOwnerEyeTraceEnt:GetClass(), "vehicle") or
        ...
        elseif (
            SWEPOwner:GetActiveWeapon() and
			SWEPOwner:GetActiveWeapon():IsValid() and
			SWEPOwner:GetActiveWeapon():GetClass() == "swep_vehicle_repair"
        ) then
            local w = 5
            local h = 2
...
if SERVER then
    util.AddNetworkString("OwnerChangedSwepVehicleRepair")
    util.AddNetworkString("AnimationFunctionSwepVehicleRepair")
    util.AddNetworkString("SWEPRepairToolVehicle_TypeOfSound")
...
if CLIENT then
    net.Receive("OwnerChangedSwepVehicleRepair", function()
        SWEPOwner = net.ReadEntity()
    end)
    net.Receive("AnimationFunctionSwepVehicleRepair", function()
		local Action = net.ReadString()

        -- Remove timers for animations..
        timer.Remove("mbd:animationFunctionToIdlePos")

        local _Weapon = LocalPlayer():GetWeapon("swep_vehicle_repair")
...
function SWEP:Deploy()
    ...
    if SERVER then
        timer.Create("mbd:OwnerChangedSwepVehicleRepair001", 0.3, 1, function()
            if net.Send then
                timer.Remove("mbd:OwnerChangedSwepVehicleRepair001")
    
                net.Start("OwnerChangedSwepVehicleRepair")
...
local avsluttAnimasjonEnd = function(self, owner)
    if SERVER then
        timer.Create("mbd:SwepVehicleRepairAnimation001", 2, 1, function()
            self:SetPrimaryIsOn(false)
            self.SetEffectPrimaryOnType(nil)
            owner:SetAnimation(PLAYER_IDLE)

            net.Start("AnimationFunctionSwepVehicleRepair", true)
                net.WriteString("end")
            net.Send(owner)
        end)
    end
...
function SWEP:PrimaryAttack()
    ...
    _Weapon:SetNextPrimaryFire(CurTime() + 2.1)
    ...
    if _Distance > 100 then self:SetPrimaryIsOn(false) avsluttAnimasjonIdle(self, _Owner) return false end
    ...
    if SERVER and ent:IsNPC() and MBD_CheckIfNotBullseyeEntity(ent:GetClass()) then
        ...
        if (
            -- Easter Egg
            _Owner:MBDShouldGetTheAdminBenefits()
        ) then d:SetDamage(1000) else d:SetDamage(6) end
        ...
        net.Start("SWEPRepairToolVehicle_TypeOfSound")
    ...
    if !string.match(_Class, "vehicle") then self:SetPrimaryIsOn(false) avsluttAnimasjonIdle(self, _Owner) return false end
    ...
    if HealthLeft >= HealthTotal then
        ...
        -- Sound Emit
		if !timer.Exists("mbd:SwepVehicleRepair001") then
            TypeOfSound = "full"
            ...
            --
            --- -- ->
            timer.Create("mbd:SwepVehicleRepair001", math.random(5, 10) / 10, 1, function()
                timer.Remove("mbd:SwepVehicleRepair001")
            end)
        end
    ...
    if (
        ...
        !self:GetCanAddHealthAgain() or (
            _Owner:IsValid() and
            (
                !_Owner:IsSuperAdmin() and -- And not a superadmin
                _Owner:GetNWInt("classInt", -1) != 1 -- Must be an Mechanic to heal all
            )
        )
    ...
    if HealthLeft < HealthTotal then
        ...
        -- -- ->>
        local newHealth = nil
        if _Owner:GetNWInt("classInt", -1) == 1 or _Owner:IsSuperAdmin() then newHealth = HealthLeft + 50 else return false end -- Mechanic (must be one)
    ...
    if !timer.Exists("mbd:SwepVehicleRepair001") then
        ...
        --
        --- -- ->
        timer.Create("mbd:SwepVehicleRepair001", math.random(5, 13) / 10, 1, function()
            timer.Remove("mbd:SwepVehicleRepair001")
        end)
    end
...
function SWEP:SecondaryAttack() end -- Don't do anything plz
...
if CLIENT then
    net.Receive("SWEPRepairToolVehicle_TypeOfSound", function()
...
function SWEP:Think()
    if CLIENT then
        ...
        if Ent && Ent:IsValid() then
            ...
            if (totalHealth != -1 and leftHealth != -1) and Ent:IsVehicle() then
        ...
        if !ASoundIsPlaying and TypeOfSound then
            ...
            timer.Simple(0.03, function()
                if TypeOfSound == "full" and self:GetPlayFullSound() then
                    ...
                    local soundString = "swep/vehicle_ok0.wav"
                    if soundTypeString == 3 then soundString = "swep/vehicle_ok1.wav" end
                    ...
                    timer.Simple( math.random( 3, 5 ), function() self:SetPlayFullSound(true) end )
                elseif TypeOfSound == "healing" then
                    local __type = math.random(0, 1)
